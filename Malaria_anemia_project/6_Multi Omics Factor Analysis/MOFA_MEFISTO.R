...
multiplex <- read.csv("Clinical_luminex_BMqRTPCR_Parasite_nanostring_scaled.csv")
...
df_to_cluster.active <- df_to_cluster[,1:1000] 
...
write.csv(mydata_complete, "Clinical_luminex_BMqRTPCR_Parasite_nanostring_scaled_complete.csv")

# Prepare data
mydata <- scale(df_to_cluster.active)
mydata2 <- cbind.data.frame(mydata, multiplex$Clinical_group)
write.csv(mydata2, "Clinical_luminex_BMqRTPCR_Parasite_nanostring_complete.csv")

# 2.3- Prepare MOFA?MEFISTO input df
mydata3 <- read.csv("Clinical_luminex_BMqRTPCR_Parasite_nanostring_scaled_complete.csv")
library(reshape)
data.m <- melt(mydata3)
write.csv(data.m, "Clinical_luminex_BMqRTPCR_Parasite_nanostring_scaled_complete_MOFA.csv")
...
mydata4 <- read.csv("Clinical_luminex_BMqRTPCR_Parasite_nanostring_scaled_complete_MOFA.csv")
...
model_opts$num_factors <- 3 #play with this
...
outfile <- "Clinical_luminex_BMqRTPCR_Parasite_nanostring_complete_MOFA.hdf5"
MOFAobject <- run_mofa(MOFAobject_untrained, outfile = outfile)


#4- Downstream Analysis
# Load the model
outfile2 <- "Clinical_luminex_BMqRTPCR_complete_scaled_MOFA_v1.hdf5"
...
group.colors <- c(
  "Severe anemia" = "salmon",
  "Moderate anemia" = "light blue")
...
plot_weights_fn <- function(MOFAobject, factor=1, view=1, nfeatures=20) {
  p1 <- plot_weights(MOFAobject, 
                     factors = factor, 
                     view = view,
                     nfeatures = nfeatures,
                     text_size = 3, dot_size = 2)
...
plot_weights_fn(MOFAobject, factor=1, view="Clinical", nfeatures=20)
plot_weights_fn(MOFAobject, factor=1, view="Parasite", nfeatures=20)
plot_weights_fn(MOFAobject, factor=1, view="PB_biomarker", nfeatures=20)
plot_weights_fn(MOFAobject, factor=1, view="BM_biomarker", nfeatures=10)
plot_weights_fn(MOFAobject, factor=1, view="BM_parasite_expression", nfeatures=20)
plot_weights_fn(MOFAobject, factor=1, view="PB_parasite_expression", nfeatures=20)
...
weights <- read.csv("Factor2_PB_parasite_expression.csv")
plot_ <- ggplot(weights,
                aes(x= reorder(description,
                               (Value), decreasing = FALSE), y = (Value), fill = stage)) +
  ...
  xlab("") + 
  ylab("Factor weight") +
  ggtitle("BM parasite expression - Integrated signature 1") +
...
write.csv(factors3,'MOFA_anemia_factor_values.csv')
...
lapply(weights,dim)
weights2 <- weights[["PB_parasite_expression"]]
write.csv(weights2,'PB_parasite_expression_factor_weight.csv')

# Add stage and description info for each gene
df1 <- read.csv("PB_parasite_expression_factor_weight.csv")
...
write.csv(df1, "PB_parasite_expression_factor_weight.csv")
