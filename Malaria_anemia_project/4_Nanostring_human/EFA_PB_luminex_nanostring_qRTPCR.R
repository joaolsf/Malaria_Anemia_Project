...
setwd("/Users/joaoluizsfilho/Dropbox/Work_Files/Matthias_Lab/Projects/Severe_Anemia_project/4_Nanostring_human/7_EFA_Nanostring_qRTPCT_luminex/PB_nanostring_qRTPCR_luminex")

# 1- Missing data imputation
library("missMDA")

multiplex <- read.csv("EFA_PB_luminex_nanostring_qRTPCR.csv")

df_to_cluster <- multiplex[,-1]
rownames(df_to_cluster) <- multiplex$PatientID

#subsetting active variables for the PCA
df_to_cluster.active <- df_to_cluster[,2:84]
...
write.csv(mydata_complete, "EFA_PB_luminex_nanostring_qRTPCR_complete.csv")
...
multiplex <- read.csv("EFA_PB_luminex_nanostring_qRTPCR_complete.csv")
...
#Small values 5.466432e-253 < 0.05) of the significance level indicate that a factor analysis may be useful with our data.
det(cor(mydata))
...
write.csv(fa.none$scores, "PB_luminex_nanostring_qRTPCR_factor_scores_iteration1.csv")
write.csv(fa.none$loadings, "PB_luminex_nanostring_qRTPCR_factor_loadings_iteration1.csv")
