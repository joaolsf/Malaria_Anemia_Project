...
multiplex <- read.csv("Cohort_anemia_luminex_data_factor_scores_iteration1_boxplot.csv")

multiplex$title <- "PB signature 4"
p <- ggplot(multiplex, aes(x=Clinical_group, y=PBS4, fill=Clinical_group,)) +
...
p + scale_fill_manual(values = c("light grey", "#0073C2FF", "salmon")) + scale_x_discrete(limits=c("Mild anemia", "Moderate anemia", "Severe anemia"))  + facet_grid(. ~ title)
