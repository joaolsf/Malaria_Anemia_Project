...
setwd("/Users/joaoluizsfilho/Dropbox/Work_Files/Matthias_Lab/Projects/Severe_Anemia_project/4_Nanostring_human/5_PCA_UMAP_heatmap")
...
multiplex <- read.csv("cohort_anemia_nanostring_human_PB_complete_heatmap.csv")

# category 1
group <- multiplex$Clinical_group
...
df_to_cluster.active <- df_to_cluster[,2:39]
...
heat <- Heatmap(mydata, cluster_columns = TRUE, cluster_rows = TRUE,
                col = colorRamp2(c(1, -0.5, 0, -0.5, -2),brewer.pal(n = 5, name = "RdYlBu"), space = "RGB"),
